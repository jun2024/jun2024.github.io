...
| 단계 | 구성 요소 | 설명 |
|------|----------|------|
| 1. 메트릭 노출 | App Pod (`/metrics`) | 애플리케이션이 Prometheus 형식으로 메트릭 엔드포인트를 노출 |
| 2. 노드 메트릭 | Node Exporter (DaemonSet) | 각 노드의 CPU, 메모리, 디스크, 네트워크 메트릭 수집 |
| 3. 수집 (Scrape) | Prometheus Server | Pull 방식으로 대상의 `/metrics` 엔드포인트를 주기적으로 가져옴 |
| 4. 저장 | TSDB (시계열 DB) | 수집된 메트릭을 시계열 데이터로 저장 |
| 5. 쿼리 | PromQL Engine | 저장된 데이터를 쿼리 언어로 조회/집계 |
| 6. 시각화 | Grafana | PromQL 결과를 대시보드로 시각화 |
| 7. 알림 | Alertmanager | 알림 규칙 충족 시 Slack, Email 등으로 알림 전송 |
...
| 방식 | 흐름 | 대표 솔루션 |
|------|------|------------|
| **Push** | App → 모니터링 서버 | Datadog, CloudWatch |
| **Pull** | 모니터링 서버 → App의 `/metrics` | Prometheus |
