...
| 단계 | 구성 요소 | 역할 |
|------|----------|------|
| 1. 로그 출력 | App Pod (stdout/stderr) | 컨테이너가 표준 출력으로 로그를 출력 |
| 2. 로그 저장 | 노드 파일시스템 (`/var/log/containers/*.log`) | kubelet이 컨테이너 로그를 노드에 저장 |
| 3. 로그 수집 | **Fluentd** (DaemonSet, 각 노드마다 1개) | 로그 파일을 읽고 파싱/필터링/태그 추가 |
| 4. 로그 저장/인덱싱 | **Elasticsearch** | 풀텍스트 인덱싱, 시계열 저장, 검색 엔진 |
| 5. 시각화 | **Kibana** | 로그 검색, 시각화, 대시보드 |
...
| 1. 로그 출력 | App Pod (stdout/stderr) | 컨테이너가 표준 출력으로 로그를 출력 |
| 2. 로그 저장 | 노드 파일시스템 (`/var/log/containers/*.log`) | kubelet이 컨테이너 로그를 노드에 저장 |
| 3. 로그 수집 | **Promtail** (DaemonSet, 각 노드마다 1개) | 로그 파일을 읽고 라벨 추가 |
| 4. 로그 저장/인덱싱 | **Loki** | 라벨 기반 인덱싱, 로그 청크 압축 저장 |
| 5. 시각화 | **Grafana** | LogQL 쿼리, 로그 + 메트릭 통합 (Prometheus와 같은 UI) |
...
| 단계 | 구성 요소 | 역할 |
|------|----------|------|
| 각 노드 | **Fluent Bit** (경량 수집) | 노드별 로그 수집, 최소한의 리소스 사용 |
| 중앙 | **Fluentd** (집계/가공/라우팅) | 여러 노드에서 받은 로그를 가공하고 라우팅 |
| 저장소 | **Elasticsearch** 또는 **Loki** | 최종 로그 저장 및 검색 |
...
### K8s Observability Stack 전체 구성

| 영역 | CKA 기본 | 실무 표준 |
|------|---------|----------|
| **Monitoring** | Metrics Server | Prometheus + Grafana |
| **Logging** | kubectl logs | EFK Stack 또는 Loki + Grafana |
| **Alerting** | - | Alertmanager (메트릭 기반) + Loki Ruler (로그 기반) → Slack, Email, PagerDuty |
| **Visualization** | - | Grafana (모니터링 + 로깅 통합 대시보드) |
