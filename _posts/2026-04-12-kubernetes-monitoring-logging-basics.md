...
| 계층 | 구성 요소 | 설명 |
|------|----------|------|
| **노드 (Node 1~N)** | kubelet + cAdvisor | 각 노드에서 컨테이너의 CPU/메모리 메트릭을 수집 |
| **집계** | Metrics Server (In-Memory) | 모든 노드의 cAdvisor 데이터를 모아서 집계 |
| **API** | kube-apiserver | Metrics Server 데이터를 API로 노출 |
| **조회** | kubectl top node / pod | API를 통해 사용자가 메트릭을 조회 |

메트릭 수집 흐름은 이래요: **각 노드의 cAdvisor** → **Metrics Server** → **kube-apiserver** → **kubectl top**
...
| 경로 | 설명 |
|------|------|
| `/var/log/containers/<pod>_<ns>_<container>-<id>.log` | 심볼릭 링크 (kubectl logs가 읽는 경로) |
| `/var/log/pods/<pod-uid>/<container>/0.log` | 심볼릭 링크 |
| `/var/lib/docker/containers/<id>/<id>-json.log` | 실제 로그 파일 |
...
| 패턴 | 방식 | 장점 | 단점 |
|------|------|------|------|
| **1. 노드 레벨 에이전트 (권장)** | 각 노드에 DaemonSet으로 Fluentd/Fluent Bit/Filebeat 배포 → 중앙 저장소(Elasticsearch, Loki)로 전송 | 앱 수정 불필요, 통합 관리 | 노드별 에이전트 리소스 소비 |
| **2. 사이드카 컨테이너** | Pod 내부에 로그 수집 전용 사이드카 추가 → 로그 파일을 stdout으로 출력하거나 외부 전송 | 앱별 세밀한 제어 가능 | Pod마다 사이드카 추가 필요 |
| **3. 애플리케이션 직접 전송** | 앱 코드에서 직접 로그 시스템으로 전송 | 가장 유연한 방식 | 앱 수정 필요, 라이브러리 의존성 |
...
| Pod 상태 | 원인 | 확인 명령어 |
|----------|------|-----------|
| **Pending** | 리소스 부족 (Insufficient CPU/Memory) | `kubectl describe pod` → `kubectl top node` |
| **Pending** | 노드 매칭 실패 | `kubectl describe pod` → Taint/Affinity 설정 확인 |
| **CrashLoopBackOff** | 앱 에러로 반복 재시작 | `kubectl logs --previous` |
| **ImagePullBackOff** | 이미지 이름/레지스트리 오류 | `kubectl describe pod` → 이미지 경로 확인 |
| **Running (동작 이상)** | 앱 내부 로직 문제 | `kubectl logs -f` |
